...
Claude 세계와 앱 세계를 연결하는 접착제가 필요하다. 사용자가 스크린샷이나 PDF를 드래그드롭하면 Claude가 메시지 content block으로 즉시 받을 수도 있고, 파일 참조가 필요한 도구 호출(Read 등)을 위해 **Claude 자식 프로세스의 CWD에 해당 파일이 실제로 존재할 때만 가능한 상대 경로**도 쓸 수 있어야 한다.

해결: 앱이 세션마다 전용 디렉토리를 만들고, **Claude 자식(`chat_host`가 관리)도 그 디렉토리에서 spawn**한다. 드래그드롭된 파일은 이 디렉토리의 `uploads/` 서브폴더에 저장되므로 Claude가 상대 경로로 즉시 참조 가능하다. 동시에 첨부 블록으로 base64 인코딩해 메시지에 태울 수도 있다.
...
좌측 ChatPanel이 드래그드롭을 받으면 두 경로를 동시에 진행한다:
1. 파일 바이트를 세션 디렉토리 `uploads/`에 저장해 Claude의 파일 도구(Read 등)가 상대 경로로 참조할 수 있게 한다.
2. 같은 바이트를 base64로 인코딩해 다음 유저 메시지의 **첨부 content block**(image/document)으로 바로 태운다.

```tsx
// src/panels/ChatPanel.tsx (발췌)
const onDrop = async (e: React.DragEvent) => {
  e.preventDefault();
  const files = [...(e.dataTransfer?.files ?? [])];
  if (files.length === 0) return;

  // 1) 세션 uploads/에 저장 (파일 도구용)
  const saved = await saveUploads(files);

  // 2) 채팅 composer의 첨부 슬롯에 추가 (메시지 content block용)
  for (const f of files) {
    await useChatStore.getState().addAttachmentFromFile(f);
  }
...
`addAttachmentFromFile`는 MIME을 보고 `image` 블록(PNG/JPEG/GIF/WebP), `document` 블록(PDF), 혹은 전개된 `text` 블록(.md/.txt 등)으로 분기한다.
...
Excalidraw의 onChange는 매 프레임 호출될 수 있어 과잉. 150ms 디바운스.
...
사용자가 "이거 한번 봐봐" 하고 싶을 때는 TopBar의 📸 버튼:

1. 버튼 클릭 → `exportToBlob` 실행
2. `previews/snapshot-{timestamp}.png` 에 저장
3. ChatComposer의 draft에 해당 이미지를 **첨부 chip으로 바로 붙이고**, draft 본문엔 `@previews/snapshot-xxx.png` 참조를 프리필
4. 사용자가 메시지 이어 작성 후 Send

Claude는 메시지 첨부(`image` content block)와 파일 참조 두 경로를 동시에 받으므로 vision 분석과 Read 도구 둘 다 가능하다.
