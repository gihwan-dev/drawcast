...
- [ ] `app`에 Tauri + Vite + React + xterm.js + `<Excalidraw />` 스켈레톤
...
### 목표
Claude Code / Codex CLI가 MCP tool을 호출하면 JSON 파일이 자동으로 갱신되는 상태. 앱 없이도 다이어그램 생성 가능.
...
### 목표
Tauri 앱이 실행되면 MCP sidecar 자동 시작 + 좌측 터미널 + 우측 Excalidraw + 실시간 동기화.
...
**3.5 Terminal Panel (1-2일)**
- [ ] xterm.js 통합
- [ ] Rust `spawn_cli` command — Claude Code / Codex 프로세스 생성
- [ ] stdin/stdout 브리지 (Tauri event)
- [ ] CLI 선택 드롭다운 (Claude Code / Codex)
- [ ] "Connect CLI" 버튼 → `~/.claude.json` 또는 `~/.codex/config.toml` 자동 등록
...
- "Connect CLI" 버튼 한 번 클릭으로 Claude Code 연결 완료
- 좌측 터미널에서 "박스 3개 그리고 화살표로 이어줘" → 우측 Excalidraw에 실시간 반영
- 세션 전환 → 다른 씬 로드
...
**6.4 첫 실행 UX (0.5일)**
- [ ] Welcome 화면 — "Connect CLI" 안내
- [ ] Claude Code / Codex 설치 여부 감지 → 미설치 시 설치 링크 표시
...
| #14 | 3.5 | Terminal panel + CLI 등록 | L |
| #15 | 3.6 | 세션 관리 | M |
| #16 | 4.1 | 파일 업로드 | M |
...
| Claude Code CLI의 stdin 처리가 xterm과 안 맞음 | Phase 3.5에서 PTY 사용 (`pty-process` 크레이트) |
...
- Claude Code / Codex의 MCP 지원이 중단되면? → stdio 모드는 표준 프로토콜이라 다른 MCP-capable CLI로 이식 가능.
