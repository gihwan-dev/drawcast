...
2. **작업 디렉토리** (모두 gitignore 됨, 커밋 대상 아님):
   - 이번 루프 노트: `evals/automation-runs/<run-id>/` — `<run-id>`는 `YYYYMMDD-HHMMSS-<shortRand>` 형식
     - `scenario.md` — 선택한 시나리오와 선정 근거
     - `findings.md` — 관찰된 문제와 가설
     - `iterations.md` — 수정 시도와 검증 결과 (반복마다 append)
   - 실행 이력: `evals/automation-runs/_history.jsonl` — 매 루프 종료 직전 한 줄 append
     ```json
...
6. 작업 노트(`evals/automation-runs/`)와 결과(`evals/results/`)는 모두 gitignore이며 커밋 대상이 아니다.
